package com.raywenderlich.pertemuan_7_1918049;
...
public class MyDatabase extends SQLiteOpenHelper {
    ...
    public int UpdateMahasiswa (Mahasiswa data){
        ...
        values.put(tb_mahasiswa_nama, data.get_nama());
        values.put(tb_mahasiswa_kelas, data.get_kelas());
        return db.update(tb_mahasiswa, values, tb_mahasiswa_id +" = ?",new String[]{String.valueOf((data.get_id()))});
    }
    public void DeleteMahasiswa(Mahasiswa data){
        SQLiteDatabase db = this.getWritableDatabase();
        db.delete(tb_mahasiswa,tb_mahasiswa_id+ " = ?",new String[]{String.valueOf(data.get_id())});
        db.close();
    }
}
